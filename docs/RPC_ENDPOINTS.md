...
Public-only (no keys), in failover order:

```
RPC_URLS=https://rpc.mainnet.chain.robinhood.com,https://robinhood.api.pocket.network,https://rpc-robinhood.blockmachine.io,https://robinhood-rpc.publicnode.com
```

With a keyed provider (preferred for the indexer and worker): put the Alchemy (or QuickNode) URL **first**, keep the public ones as fallbacks. Never commit keys; set them on Railway / Supabase.
