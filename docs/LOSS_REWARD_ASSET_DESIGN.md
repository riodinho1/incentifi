...
## C11. Fee plumbing is not automatic: worker collect()/convert(), creator two-step claim, stock claim display (2026-09-08)

**Observed on TESSSS (`0x8976d274…`, Loss Reward: GOOGL):** trades happened, yet `creatorBalances` and `V2.totalDeposited(token)` stayed 0. On a legible pool the 2 % LP fee accrues **inside the hook's PoolManager position**; nothing moves until `hook.collect(token)` (ETH half → `creatorBalances` / `LossRewardPoolV2.depositReward`, token half → converter) and then `converter.convert(token, 0, 0)` (token-side fees sold for ETH, split the same way). Nobody was calling either. Read back on 2026-09-08: TESSSS had ≈ 0.00258 ETH + ≈ 827 k tokens uncollected in the position; `collect()` estimates at 288 048 gas ≈ 0.000076 ETH.

**Simulating `collect()` without a transaction.** `collect()` returns nothing and `eth_call` returns no logs, so the frontend and the worker apply the PoolManager's own formula to public state: `fees_i = mulDiv(feeGrowthInside_i − feeGrowthInside_iLast (mod 2²⁵⁶), liquidity, Q128)` from `StateView.getPositionInfo(poolId, hook, tickLower, tickUpper, salt)` + `getFeeGrowthInside(poolId, tickLower, tickUpper)` (curve position `[174070, 200310]`, salt 0; full range, salt 1 after graduation). `src/lib/legibleFees.ts` / `scripts/lib/legiblePool.mjs` (`computeUncollectedLegibleFees`, `uncollectedFeesFromGrowth`, `splitCollectedFees`). The fork test proves the prediction equals the real `FeesCollected` figures to the wei.

**Worker (`collectLegibleFees`, every tick after the epochs, before the fallback monitor).** For every legible token with a trade in the last `FEE_COLLECT_LOOKBACK_HOURS` (24; `token_trades_evm.block_time`, legible = `tokens.hook_address` tag or on-chain check): simulate `collect()`; if ETH fees ≥ `FEE_COLLECT_MIN_MULTIPLE` (10) × (estimated gas × gas price) send `collect(token)` from the operator; then read `converter.pendingTokenFees(token)`, value it with `checkpointEthValue`, and send `convert(token, 0, 0)` on the same rule (the converter enforces its own checkpoint floor). Gas = estimate + 30 %, never below 300 000. Both are logged with the receipt figures; a failure alerts and never delays a payout. `FEE_COLLECT_ENABLED=false` switches it off; `dryRun` decides without sending. Tests: `test/worker-fee-collection.test.mjs` (offline, decodes every signed tx), `test/hardhat/legible-fees-fork.test.ts` §5 (live contracts).
...
## C13. Underfunded epochs pay what the pool holds; `pending_funding` retired (2026-09-09)

Incident: INCENTIFI (reward asset NVDA) had 0.2923 ETH unallocated on V2 and two `pending_funding` epochs (#4 = 0.3024 ETH, #5 = 0.2479 ETH) that never published; Claimable was 0 for every holder. The worker's funding gate read the right number (`getUnallocatedBalance(token)` on the epoch's pool, the same call the site shows as "unallocated"; it recorded 0.2387 ETH, not zero). The rule was wrong: an underfunded epoch was parked at FULL demand, the resolver required the FULL amount FIFO, so #4 (0.01 ETH short) blocked #5 for good while every new 5-minute epoch parked another full-demand row, and step 13 had already depleted each holder's cost basis for money never paid. A 10%-of-loss demand every 5 minutes can never be caught up by a 1%-of-volume pool.

Now (`scripts/loss-reward-worker.mjs`):

- **Every epoch pays what the pool holds now, pro rata** (`capAllocationsToAvailable`, the V1-drain rule of PR #23, on every pool). With 0 available the dust guard records `completed_dust`; `pending_funding` is no longer created and cost basis is depleted only for a `published` epoch.
- **Legacy pending rows are published by `resolvePendingEpochs()`**, oldest first, from their stored `epoch_holder_rewards` amounts (no re-snapshot): scaled pro rata to the pool, tree and proofs rewritten, the unpaid part of each holder's earlier depletion given back to `holder_cost_basis`, roots already on-chain reconciled without a new tx. While a token still has pending rows no new epoch is computed for it (they would compete for the same fees). INCENTIFI #4 publishes for ~0.2923 ETH on the next cycle; #5 follows as fees arrive; CLUB's 23 pending rows drain one per run.
- **The pool read is retried** (`readUnallocatedWithRetry`, `POOL_READ_ATTEMPTS`=3); after the last attempt the token is skipped for the run (`pool_read_failed` / `pending_resolution_failed`) instead of the old "default to demand", which produced a publish the contract reverted.
- **Leaf amounts are float round-trip stable** (`stabilizeLeafWei`): `epoch_holder_rewards.final_reward_eth` is a NUMBER and the gateway rebuilds the claim amount as `Math.round(eth * 1e18)`; above ~2^52 wei that differs from the hashed wei and the proof fails on-chain. Each leaf is now the fixed point of that round trip (sum kept ≤ the pool with a 1e-12 ETH slack).

Test: `test/worker-pending-epoch-funding.test.mjs` (production state reproduced offline). Open follow-ups seen during the diagnosis: the worker's freshness gate checks the heartbeat's age but not its `status` (the indexer was `error: V4 discovery not ready` while epochs kept paying on unrefreshed cost basis), and no epoch row was written for any token after 22:58 UTC on 2026-09-08 (Railway logs needed).


## C14. Balance reads confirmed, pending epochs rebuilt from chain, idempotent trades, holder reconciliation (2026-09-09)

**What was actually wrong (verified on chain before changing anything).** The balance guard's "on-chain 0" for INCENTIFI's creator `0xb45e…c4a1` was *correct at the time*: its Transfer history shows it sold everything at 22:10:55 UTC and only received its current 21.4M tokens at 22:47:24 UTC, after both epoch snapshots (22:08, 22:33). Every endpoint returns 21.4M today, and viem throws on empty or short call data, so a bad RPC can only ever produce this line as a well-formed zero word from a node serving stale or foreign state — which is what the new read path defends against. The "exactly half" rows are real DB damage: holder `0x6e5e24f8…` has ONE buy of 4,159,194 on chain and ONE trade row, but a DB balance of 8,318,387. `processBuyTrade` updated `holder_cost_basis` *before* writing the trade row, so a failure between the two re-applied the buy on the next rescan. Trade rows themselves cannot duplicate (`tx_hash` = `<tx>:<logIndex>` is the primary key; 0 legacy ids in production). The 89 INCENTIFI holders at "chain 0" are sells the indexer never indexed: its trade cursor froze at 23:00:42 UTC when V4 discovery got stuck rescanning after a restart. The pool figures 0.2387 (worker, 22:33) and 0.2923 (site, later) were both right: the same `getUnallocatedBalance` at different times, with a 0.0536 ETH deposit landing at 22:46:26 UTC in between.

**Changes.**

- `scripts/lib/reliableBalance.mjs` — `readBalanceReliably()`: every `balanceOf` is pinned to one block per run (never `latest`); a result *below* the DB balance (a zero included) is confirmed on a **different endpoint** of the failover provider (`rpcFailover.requestVia`), or re-read after a delay when only one endpoint is configured; both agree → accepted, they disagree → `disputed`. The worker **skips a disputed holder** for that epoch (not paid, not zeroed, DB untouched, loud `[BALANCE GUARD] DISPUTED` line) and **skips the token** when a read fails on every endpoint. The failover provider now also rejects a JSON-RPC answer whose `id` is not the request's.
- **Pending epochs are rebuilt, not republished.** `reversePendingEpochs()` gives back the cost-basis depletion every legacy `pending_funding` row applied and deletes its stored per-holder rows (row by row, restore-then-delete, so a crash never restores twice); `executeEpochForToken` then rebuilds the oldest pending epoch *in place* (same `epoch_id`/number, `upsert` on `(token_address, epoch_number)`) from fresh confirmed reads — anyone who fully sold gets nothing — and funds it pro rata. One pending epoch per run; new numbers only once none is pending.
- **Idempotent trade application** (`claimTrade`): trade row first with `applied = false` → holder update → `applied = true`; a rescan re-applies only rows still `applied = false`. Migration `supabase/trade_idempotency_and_reconciliation.sql` adds the column (the indexer still works without it, trade-row-first) and the audit table.
- **Holder reconciliation** (`reconcileHolderBalances`, every `HOLDER_RECONCILE_INTERVAL_MS` = 30 min from the indexer tick, at the block just indexed — not head, so a not-yet-indexed sell is never mistaken for an error): a DB balance above the confirmed chain balance is lowered to it with the investment scaled by the same ratio (average cost preserved); chain above DB is logged only; disputed reads change nothing; every change is audited in `holder_balance_reconciliations`.
- The worker's GenericSell price path (`fetchV4CurveState` via Vite SSR) now takes the failover client; `src/lib/bondingCurveV4.ts` accepts an optional `client`.
- Logging: every line of an epoch run is prefixed `[<token address>]`; `[SUCCESS] … Distributed` only for a published epoch (`[SAVED] … stored as '<status>'` otherwise); the pool line names the pool address and the run's snapshot block.

Tests: `test/worker-balance-read-guard.test.mjs` (endpoint A says 0, B says 1,000,000 → holder skipped, not zeroed; read fails everywhere → token skipped; both say 0 → real exit), `test/worker-pending-epoch-funding.test.mjs` (rebuild semantics), `test/indexer-trade-idempotency.test.mjs` (double-apply, half-applied row, reconciliation at a pinned block), `test/rpc-failover.test.mjs` 7b (foreign id, `requestVia`).

**Before restarting the worker:** run the migration, deploy the indexer first and let its reconciliation pass (`[RECONCILE] done`) lower the stale INCENTIFI/CLUB balances, then start the worker. Its first run per token reverses the pending depletion and rebuilds the oldest pending epoch; INCENTIFI has 2 pending rows, CLUB 23, `0x3374fe8f…` 5.
