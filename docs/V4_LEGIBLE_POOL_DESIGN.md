...
**Status (updated 2026-09-08):** DEPLOYED on Robinhood Chain 2026-09-07 (§12) and cut over for new launches (§13); the loss-reward pointer was re-pointed to LossRewardPoolV2 the same day. Numbers below were corrected from the suite (tickSpacing 10, see §3.1). Decision D is **final** as of review round 2: the 2% fee continues after graduation, capped at 2%, no timelock. **Owner today: a software key in a Foundry keystore (`0x78a4E4BC…`), which is also the worker's operator key** — the hardware-wallet owner + separate operator key are the open follow-ups in `docs/AUDIT_2026-09-08.md` (§G, follow-ups).
...
Earlier drafts shipped the post-graduation fee at 0 behind a governed switch ("one-way door"), then added a 2-day timelock and a multisig-owner requirement. **All of that is superseded** by the product decision in §3.6: the fee is 2% throughout, on from launch, capped at 2% in the contract, adjustable within `[0, 2%]` immediately. Because the cap equals the default, the owner cannot raise anything; the only remaining owner levers are lowering the fee (helps traders) and opening external LP on graduated pools (decision C). With that little power, **the owner is a single EOA, deliberately** (a software key in a Foundry keystore as of 2026-09-08; moving it to a hardware wallet via `transferOwnership` is an open follow-up) — `script/DeployLegiblePool.s.sol` deploys with a plain EOA owner (the `--sender`, or `OWNER` if set). A hook fee is enforced on every path, so unlike the V3 post-grad fee it cannot be bypassed by trading the pool directly.
...
| **D. Post-grad fee level** | 0 · 1%/1% · governed | **RESOLVED (final): 2% (1%/1%) throughout, on from launch, capped at 2%, no timelock; owner is a single EOA by choice (software keystore today, hardware wallet pending)** (§3.6, §6) |
...
2. Mainnet: mine salt for `0x28C0`, deploy hook → `FeeConverter` → factory, re-point `uniswapAddresses.ts` + indexer factory default, PR to master (user runs deploys with own key; each tx verified on-chain). Owner: the deploying EOA (or `OWNER`), by choice — see §6; no multisig, no timelock. (Done 2026-09-07 from the `incentifi-owner` software keystore; the re-point of `uniswapAddresses.ts`/indexer happened through flags, PR #21, not defaults.)
...
Wiring (`setFactory` tx `0xb260214d…`, `setFeeConverter` tx `0x26000279…`), read back: `hook.factory()` / `hook.feeConverter()` / `factory.hook()` / `converter.hook()` match the table; `hook.owner() == hook.deployer() == 0x78a4E4BCC8ab559B6d3B1Cb9eab0A04a2411c726` (a software-keystore EOA, §6 — hardware wallet pending); `hook.lossRewardPool() == converter.lossRewardPool()` read `0x697BDA9d…` (V1) at deployment and **`0x5d94246C…` (LossRewardPoolV2) since the re-point tx `0xfbc1bd47…` on 2026-09-07**; `PRE_GRADUATION_FEE_PIPS == DEFAULT_POST_GRADUATION_FEE_PIPS == MAX_POST_GRADUATION_FEE_PIPS == 20 000`; `TICK_SPACING 10`, bounds `[174070, 200310]`; factory `POOL_FEE == DYNAMIC_FEE_FLAG`. Source verification, all with the exact `foundry.toml` settings (solc v0.8.26+commit.8a97fa7a, optimizer 200 runs, viaIR, cancun) from the standard-JSON input `forge verify-contract --show-standard-json-input` produces:
...
| Converter `0xe1BB0667…F7D9` | **exact match**, 14:54:11Z — https://sourcify.dev/server/v2/contract/4663/0xe1BB0667d64683072BaeE03D8D9Feb201dcAF7D9 | **pending as of 2026-09-08** (submitted again by the audit remediation; see `docs/AUDIT_2026-09-08.md` §A5 for the live status of all six contracts). Sourcify is the verification that counts. |

**§5 acceptance — done 2026-09-07/08:** smoke test passed (SMK95868), DexScreener indexes both the smoke token and TESSSS on chain `robinhood`, the frontend/indexer cutover shipped behind flags (PR #21, now ON in production) and the re-point to V2 happened. GMGN/Axiom could not be verified programmatically (Cloudflare / 403). The original plan, for the record: run `scripts/smoke-test-legible-pool.mjs` from a funded throwaway wallet (launch → UR buy → UR sell → `collect` → `convert`); confirm real `Swap` events and a moving `slot0`; DexScreener indexes the poolId; Quoter == UniversalRouter; GMGN / Axiom show the token. Only then re-point `src/lib/uniswapAddresses.ts` and the indexer's factory default to this trio.
