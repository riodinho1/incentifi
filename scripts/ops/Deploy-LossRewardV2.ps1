...
.DESCRIPTION
  Steps, in order (each is a separate invocation; every step re-checks the chain before acting):
    ...
    Preflight          read-only. Chain id, hook owner, current loss-reward pool (V1 before the
                       2026-09-07 re-point, V2 after - both reported, neither fails), route pools
                       (token0 == WETH, token1 == asset, fee tier), StockFactory round-trip for
                       AAPL/TSLA/NVDA, registry not paused, owner balance, forge build.
    ...
    SplitOperatorDryRun -NewOperator 0x...   forge script SetOperator.s.sol WITHOUT --broadcast: V1.setOperator +
                       V2.setOperator to a fresh key so the Railway worker key can only publish epochs
                       (audit 2026-09-08 finding 1). Refuses the owner itself and the zero address.
    SplitOperatorBroadcast -NewOperator 0x...  the same with --broadcast; reads both pools back and prints
                       the exact Railway variable to change (OPERATOR_PRIVATE_KEY = the NEW key's private key).
    SetRouteEnabledDryRun / SetRouteEnabledBroadcast -V2 0x... -Assets QUBT,BE -Enabled false
                       forge script SetAssetEnabled.s.sol: disable (or re-enable) routes whose venue lost
                       its TWAP or liquidity. Symbols resolve through config/loss-reward-stock-routes.json
                       (or its overrides file); addresses are accepted too.
...
.EXAMPLE
  ...
  .\scripts\ops\Deploy-LossRewardV2.ps1 -Step SplitOperatorDryRun -NewOperator 0x...
  .\scripts\ops\Deploy-LossRewardV2.ps1 -Step SplitOperatorBroadcast -NewOperator 0x...
  .\scripts\ops\Deploy-LossRewardV2.ps1 -Step SetRouteEnabledDryRun -V2 0x... -Assets QUBT,BE -Enabled false
  .\scripts\ops\Deploy-LossRewardV2.ps1 -Step SetRouteEnabledBroadcast -V2 0x... -Assets QUBT,BE -Enabled false
#>
[CmdletBinding()]
param(
  [Parameter(Mandatory = $true)]
  [ValidateSet('Preflight', 'DeployDryRun', 'DeployBroadcast', 'ReadBack', 'VerifySource', 'RoutesDryRun', 'RoutesBroadcast', 'RepointDryRun', 'RepointBroadcast', 'SplitOperatorDryRun', 'SplitOperatorBroadcast', 'SetRouteEnabledDryRun', 'SetRouteEnabledBroadcast')]
  [string]$Step,
  [string]$V2 = '',
  [string]$Swapper = '',
  [string]$NewOperator = '',
...
$SET_OPERATOR_SCRIPT = 'script/SetOperator.s.sol'
$SET_ENABLED_SCRIPT  = 'script/SetAssetEnabled.s.sol'
$ROUTES_OVERRIDES    = 'config/loss-reward-stock-routes.overrides.json'   # hand-maintained: disabled routes + reasons
...
$script:gatePassed = ''
# Symbols (QUBT,BE) or addresses -> checksummed addresses via the routes file (+ overrides file).
function Resolve-AssetList($list) {
  $known = @{}
  foreach ($f in @($ROUTES_FILE, $ROUTES_OVERRIDES)) {
    if (Test-Path $f) {
      $cfg = Get-Content $f -Raw | ConvertFrom-Json
      foreach ($r in @($cfg.routes) + @($cfg.disabled)) { if ($r -and $r.symbol -and $r.asset) { $known[$r.symbol.ToUpper()] = $r.asset } }
    }
  }
  $out = @()
  foreach ($item in ($list -split ',')) {
    $t = $item.Trim(); if ($t -eq '') { continue }
    if ($t -match '^0x[0-9a-fA-F]{40}$') { $out += $t; continue }
    if ($known.ContainsKey($t.ToUpper())) { $out += $known[$t.ToUpper()]; continue }
    throw "Unknown asset '$t' (not an address and not a symbol in $ROUTES_FILE / $ROUTES_OVERRIDES)"
  }
  if ($out.Count -eq 0) { throw '-Assets is required (comma-separated symbols or addresses)' }
  return $out
}
...
switch ($Step) {
  ...
  'Preflight' {
    ...
    Check-Chain
    if (Same $script:hookPool $V1) { Pass "hook.lossRewardPool() == V1 (not yet re-pointed: the Deploy* / Repoint* steps apply)" }
    else { Info "hook.lossRewardPool() = $($script:hookPool): already re-pointed (2026-09-07). Deploy*/Repoint* steps are DONE; use ReadBack, Routes*, SetRouteEnabled*, SplitOperator*." }
    Check-Routes
    Info ""
  ...
  'SplitOperatorDryRun' {
    RequireAddress $NewOperator 'NewOperator'
    if (Same $NewOperator $OWNER) { throw 'NewOperator must NOT be the owner key: the whole point is to take the operator role off it.' }
    Check-Chain
    Head "forge script $SET_OPERATOR_SCRIPT (SIMULATION, no --broadcast)"
    Info "V1.operator = $(CastCall $V1 'operator()(address)')  V2.operator = $(CastCall $V2 'operator()(address)')  -> $NewOperator"
    $env:NEW_OPERATOR = $NewOperator; $env:POOL_V1 = $V1; $env:POOL_V2 = $V2
    $senderArgs = Get-DryRunSenderArgs
    try { $out = & forge script $SET_OPERATOR_SCRIPT --rpc-url $Rpc @senderArgs 2>&1 | Out-String } finally { Clear-Key }
    Write-Host $out
    if ($LASTEXITCODE -ne 0) { Fail "simulation failed (exit $LASTEXITCODE)" }
    Expect-InOutput $out "operator AFTER\s+$NewOperator" "both pools would move to $NewOperator"
    Expect-InOutput $out 'operator split done' 'post-condition read-back passed in simulation'
    Info "WRONG looks like: NewOperatorIsOwner (you passed the owner), Unauthorized (the pool's onlyOwner: sender is not the pool owner), ZeroAddress."
    ExitWithSummary
  }

  'SplitOperatorBroadcast' {
    RequireAddress $NewOperator 'NewOperator'
    if (Same $NewOperator $OWNER) { throw 'NewOperator must NOT be the owner key.' }
    Check-Chain
    Head "forge script $SET_OPERATOR_SCRIPT --broadcast (2 transactions from ${OWNER}: V1.setOperator, V2.setOperator)"
    Info "After this the Railway worker MUST run with the NEW operator's key; the owner key leaves Railway entirely."
    Confirm-Typed 'SPLIT-OPERATOR' "Set the operator of V1 ${V1} and V2 ${V2} to ${NewOperator}?"
    Assert-Gate 'SPLIT-OPERATOR'
    $env:NEW_OPERATOR = $NewOperator; $env:POOL_V1 = $V1; $env:POOL_V2 = $V2
    $wallet = Get-WalletArgs
    try { $out = & forge script $SET_OPERATOR_SCRIPT --rpc-url $Rpc --broadcast --slow @wallet 2>&1 | Out-String } finally { Clear-Key }
    Write-Host ($out -replace '--private-key\s+0x[0-9a-fA-F]{64}', '--private-key <redacted>')
    if ($LASTEXITCODE -ne 0) { Fail "broadcast failed (exit $LASTEXITCODE) - re-run the dry run; the script is idempotent" }
    Head "Post read-back"
    Check 'V1.operator()' (CastCall $V1 'operator()(address)') $NewOperator
    Check 'V2.operator()' (CastCall $V2 'operator()(address)') $NewOperator
    Check 'V2.owner() unchanged' (CastCall $V2 'owner()(address)') $OWNER
    Info ""
    Info "RAILWAY (worker service): OPERATOR_PRIVATE_KEY = <private key of ${NewOperator}>   <- the ONLY variable to change"
    Info "  then redeploy the worker, confirm '[OPERATOR] ${NewOperator} balance ...' in its log, and fund ${NewOperator}"
    Info "  (epoch publish ~80k gas, collect ~180k, convert ~250k; keep >= 3 days of runway - the worker alerts below that)."
    Info "  Remove the owner key from every Railway variable. Commit broadcast/SetOperator.s.sol/4663/run-latest.json."
    ExitWithSummary
  }

  'SetRouteEnabledDryRun' {
    RequireAddress $V2 'V2'
    $assets = Resolve-AssetList $Assets
    Check-Chain
    Head "forge script $SET_ENABLED_SCRIPT (SIMULATION, no --broadcast): enabled=$Enabled for $($assets -join ', ')"
    foreach ($a in $assets) { Info "$a route: $(CastCallArgs $V2 'assetRoute(address)((address,address,uint24,uint32,uint16,bool))' @($a)) selectable: $(CastCallArgs $V2 'isSelectableAsset(address)(bool)' @($a))" }
    $env:POOL = $V2; $env:ASSETS = ($assets -join ','); $env:ENABLED = $Enabled.ToLower()
    $senderArgs = Get-DryRunSenderArgs
    try { $out = & forge script $SET_ENABLED_SCRIPT --rpc-url $Rpc @senderArgs 2>&1 | Out-String } finally { Clear-Key }
    Write-Host $out
    if ($LASTEXITCODE -ne 0) { Fail "simulation failed (exit $LASTEXITCODE)" }
    Expect-InOutput $out 'changed\s+\d+' 'logs the number of routes to change'
    Info "WRONG looks like: NoRoute(asset) (no route configured for that asset - nothing to disable), 'read-back mismatch', Unauthorized (sender is not the owner)."
    ExitWithSummary
  }

  'SetRouteEnabledBroadcast' {
    RequireAddress $V2 'V2'
    $assets = Resolve-AssetList $Assets
    Check-Chain
    Head "forge script $SET_ENABLED_SCRIPT --broadcast: enabled=$Enabled for $($assets -join ', ') (one tx per changed route from $OWNER)"
    Confirm-Typed 'SET-ROUTE-ENABLED' "Set enabled=${Enabled} on ${V2} for $($assets -join ', ')?"
    Assert-Gate 'SET-ROUTE-ENABLED'
    $env:POOL = $V2; $env:ASSETS = ($assets -join ','); $env:ENABLED = $Enabled.ToLower()
    $wallet = Get-WalletArgs
    try { $out = & forge script $SET_ENABLED_SCRIPT --rpc-url $Rpc --broadcast --slow @wallet 2>&1 | Out-String } finally { Clear-Key }
    Write-Host ($out -replace '--private-key\s+0x[0-9a-fA-F]{64}', '--private-key <redacted>')
    if ($LASTEXITCODE -ne 0) { Fail "broadcast failed (exit $LASTEXITCODE)" }
    Head "Post read-back"
    foreach ($a in $assets) { Check "isSelectableAsset($a)" (CastCallArgs $V2 'isSelectableAsset(address)(bool)' @($a)) ($Enabled.ToLower()) }
    Info "Record the change in $ROUTES_OVERRIDES (disabled list + reason) so the generator keeps it out of the next config, then regenerate src/lib/stockRewardCandidates.generated.json and deploy the frontend."
    ExitWithSummary
  }
